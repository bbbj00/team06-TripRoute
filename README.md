...
> 여행 도시와 취향을 자연어로 입력하고 이동수단을 선택하면, 공공데이터·이동정보 API·RAG·LLM Agent를 활용해 시간대별 여행 일정표, 동선 메모, 예상 비용을 자동 생성하는 Agentic Workflow 프로젝트입니다. 로컬 실행 확인은 중간 단계일 뿐, **완성된 Agent를 Docker로 컨테이너화하고 CI/CD 파이프라인을 구성해서, 실제로 동작하는 서비스로 GCP에 배포하는 것**이 최종 목적입니다 (GCP Compute Engine(GCE) VM에 배포 완료, GitHub Container Registry(GHCR) + GitHub Actions CI/CD 연동).
...
| 대화 세션 저장     | 로그인 사용자별 대화 세션/메시지 저장 및 사이드바 최근 대화 목록 |
...
| 로컬/유명 장소 필터링 | Google Places 평점·리뷰수 기반으로 로컬 선호/인기 장소 구분 |
| 연관 관광지 조회    | TourAPI 여행코스 데이터 기반 함께 방문하기 좋은 관광지 후보 탐색 |
| 동선 설계        | 카카오모빌리티 API 기반 장소 간 거리와 이동시간 반영     |
| 일정표 생성       | 시간대별 여행 일정표 생성                      |
| 예상 비용 계산     | 교통비, 식비, 입장료, 숙박비 등 예상 비용 산정        |
| 멀티턴 후속 요청    | 직전 일정을 유지한 채 "맛집 위주로", "3일로 늘려줘" 등 조건 일부만 재구성 |
...
| Auth              | Supabase Auth           | 회원가입/로그인 및 사용자별 대화 세션 구분  |
| Public Data API   | 한국관광공사 TourAPI          | 관광지 기본 정보 및 여행코스(연관 관광지) 조회 |
| Place Rating API  | Google Places API (New) | 평점·리뷰수 기반 로컬/유명 장소 필터링    |
...
| Container         | Docker, GHCR            | 멀티스테이지 이미지 빌드 및 이미지 레지스트리 |
| CI/CD             | GitHub Actions          | Lint/테스트 → 이미지 빌드·push → 배포 자동화 |
| Infra             | GCP Compute Engine(GCE) | Docker Compose 기반 프로덕션 배포  |
...
* 회원가입/로그인 (Supabase Auth) 및 로그인 사용자별 대화 세션 저장
* 멀티턴 후속 요청 (직전 일정을 유지한 채 조건 일부만 재구성)
* 자연어 여행 조건 입력
* 이동수단 선택
* 인원수 입력
* 관광지 후보 조회
* RAG 기반 취향 매칭
* Google Places 평점/리뷰수 기반 로컬/유명 장소 필터링
* 카카오모빌리티 기반 자동차 기준 거리/시간 조회
* 대중교통 휴리스틱 추정
* 시간대별 일정표 생성
* 예상 비용 출력
* Gradio 기반 UI (FastAPI에 통합, GCP GCE VM에 배포)
* FastAPI 기반 실행 엔드포인트
...
* [x] 배포 대상 서비스 확정 — **GCE(Compute Engine) VM** 한 대에 Docker Compose로 배포 (Cloud Run 아님). FastAPI 한 컨테이너 안에서 `/ui` 경로로 Gradio까지 함께 서빙
* [x] `Dockerfile` 작성 (멀티스테이지 빌드) 및 컨테이너 실행 확인
* [ ] `.env`의 API Key/Secret을 GCP Secret Manager로 이관 — 아직 미완료. 현재는 GCE VM의 `/opt/triproute/.env` 평문 파일을 `docker-compose.prod.yml`이 그대로 읽는 방식
* [x] CI/CD 파이프라인 구성 — GitHub Actions 2단계: `ci.yml`(ruff lint + pytest) → `cd.yml`(Docker 이미지 빌드 → **GHCR**(GitHub Container Registry) push → SSH로 GCE VM에 `docker compose up` 배포 → 헬스체크)
* [x] 배포 확인 — VM 내부 헬스체크(`curl localhost:8000`) 통과, 외부 접속 URL(`http://34.50.22.20:8000/ui`)로 시연 가능
