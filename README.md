...
> 여행 도시와 취향을 자연어로 입력하고 이동수단을 선택하면, 공공데이터·이동정보 API·RAG·LLM Agent를 활용해 시간대별 여행 일정표, 동선 메모, 예상 비용을 자동 생성하는 로컬 실행형 Agentic Workflow 데모 프로젝트입니다.
...
본 프로젝트는 실제 상용 여행 예약 서비스가 아니라, **로컬 환경에서 실행 가능한 Agentic Workflow MVP 데모**를 목표로 합니다.
