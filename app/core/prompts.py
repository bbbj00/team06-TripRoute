...

{
  ...
  "daily_preferences": [
    {"day": 2, "travel_style": ["취향1"], "schedule_intensity": "빡빡한 일정 또는 null"}
  ]
}
...
target_day/target_time_slot을 쓰지 않고(둘 다 null) 이 4개 필드만 채워라 — 반대로
target_day/target_time_slot을 쓸 때는 이 4개 필드를 전부 null로 둬라. 두 그룹은
동시에 채우지 않는다.

- move_source_day/move_destination_day: 문장에 며칠차인지 숫자가 둘 다 명시돼야 채운다
  (하나만 언급되면 "이동"이 아니라 슬롯 교체나 다른 요청일 수 있으니 둘 다 null로 둬라).
...
마지막날은 여유롭게"처럼 **여행을 처음 계획할 때 일차별로 다른 취향/일정 강도**를
지정한 경우에만 채운다. 이미 만들어진 일정을 나중에 "2일차만 액티비티로 바꿔줘"처럼
후속으로 바꾸는 요청에는 쓰지 않는다(그건 이번 스키마에서 지원하지 않으니 무시해라).
...
- 일차별로 다른 조건이 언급되지 않은 평범한 요청이면 daily_preferences는 빈 리스트 []로
  둬라. 언급된 날짜만 항목으로 넣고, 언급 안 된 날짜는 아예 넣지 마라.
"""
