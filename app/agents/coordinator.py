...
def run_triproute_coordinator(
    ...
    # 4. warnings 구성
    warnings.append("현재 응답은 일부 Mock 데이터 기반입니다.")
    warnings.append("대중교통 시간과 비용은 참고용 추정치입니다.")
    ...
    react_trace = [
        {
            ...
            "description": "Solar API 또는 Mock parser로 사용자 입력을 여행 조건으로 구조화",
            "parser": parsed.get("_parser", "unknown"),
        },
        {
            "step": 2,
            "action": "search_places",
            "description": "사용자 여행 조건을 바탕으로 주요 관광지 후보를 검색",
        },
        {
            "step": 3,
            "action": "get_related_places",
            "description": "선택된 관광지와 함께 방문하기 좋은 연관 장소를 조회",
        },
        {
            "step": 4,
            "action": "get_route_info",
            "description": "장소 간 이동 시간과 동선 정보를 생성",
        },
        {
            "step": 5,
            "action": "estimate_cost",
            "description": "교통비와 여행 예상 비용을 계산",
        },
        {
            "step": 6,
            "action": "build_final_response",
            "description": "Coordinator가 최종 응답을 조립",
        },
    ]
